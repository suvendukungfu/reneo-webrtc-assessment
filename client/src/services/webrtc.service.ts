...
export class WebRTCService {
  ...
  /**
   * Adds or updates local stream tracks on the active connection
   */
  ...
  /**
   * Drains any ICE candidates that arrived before setRemoteDescription was completed
   */
    ...
    while (this.pendingIceCandidates.length > 0) {
      ...
      if (candidate) {
        try {
          ...
          console.error('[WebRTCService] Error adding queued candidate:', err);
        }
      }
    }
  }
